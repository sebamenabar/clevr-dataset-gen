...
parser.add_argument('--output_blend_dir', default='output/blendfiles',
    help="The directory where blender scene files will be stored, if the " +
         ...
         "not already exist.")
parser.add_argument('--output_stats_dir', default='../output/stats',
    help="The directory where blender scene files will be stored, if the " +
         "user requested that these files be saved using the " +
         "--save_blendfiles flag; in this case it will be created if it does " +
         "not already exist.")
...
def main(args):
  ...
  for i in range(args.num_images):
    ...
    used_pixels, positions = render_scene(args,
    ...
    plt.imshow(Zi)
    plt.colorbar()
    plt.savefig(osp.join(args.output_stats_dir, 'density_%d.png' % args.start_idx))
    plt.close()

    plt.imshow(pos_density)
    plt.colorbar()
    plt.savefig(osp.join(args.output_stats_dir, 'pix_usage_%d.png' % args.start_idx))
    plt.close()

    all_positions += positions
    _p = np.array(all_positions)
    xs, ys = rotate(_p[:,0], _p[:,1], -40)
    plt.hist2d(ys, xs, (50, 50), cmap=plt.cm.jet)
    plt.colorbar()
    plt.gca().invert_yaxis()
    plt.savefig(osp.join(args.output_stats_dir, 'positions_%d.png' % args.start_idx))
    plt.close()
...
def check_visibility(blender_objects, min_pixels_per_object):
  ...
  path = None
  object_colors, path = render_shadeless(blender_objects, path=path)
...
def render_shadeless(blender_objects, path='flat.png'):
  ...
  new_path = old_filepath.replace('images', 'shadeless')

  # Override some render settings to have flat shading
  render_args.filepath = new_path
